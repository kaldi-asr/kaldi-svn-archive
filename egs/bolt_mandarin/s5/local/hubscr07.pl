...
# lw519: add mandarin encoding option '-m MandarinEncoding'
my $Usage="hubscr07.pl [ -R -v -L LEX ] [ -M LM | -w WWL ] -g glm -l LANGOPT -m MandarinEncoding -h HUBOPT -r ref hyp1 hyp2 ...\n".
"Version: $Version\n".
...
#############     Set all Global variables         #############
    ...
    ### empty.
    my $SCTK = "../../../tools/sctk";
    #"/home/widowb/ears/scoring/software/sctk-1.2c";
    my $SCLITE = "";
    my $SC_STATS = "";
    ### Installation directory for the tranfilt package.   If the package's
    ### executables are accessible via your path, this variable may remain 
    ### empty.
    my $TRANFILT="tools/tranfilt-1.14";
    my $CSRFILT="";
    my $DEF_ART="";
    my $GLM = "";
    my $ACOMP = "";
...
sub ProcessCommandLine{
    ...
    getopts('vRl:m:h:r:g:L:n:e:M:w:');

    if (defined($main::opt_l)) {	$Lang = $main::opt_l; $Lang =~ tr/A-Z/a-z/; }
    if (defined($main::opt_m)) {	$ManEncoding = $main::opt_m; }
...
sub VerifyResources{
	...
	$SCLITE = "$SCTK/src/sclite/sclite";
	$SC_STATS = "$SCTK/src/sclite/sc_stats";
    } else {
	if ($Vb){
...
sub RunScoring{
    ...
    # my $command = "$SCLITE -r $reff stm -h $hyp_oname ctm $hyp_iname -F -D -o sum pralign rsum sgml lur dtl pra -C det sbhist hist $outname";
    my $command = "../../../tools/sctk-2.4.8/bin/sclite -r $reff stm -h $hyp_oname ctm $hyp_iname -F -D -o sum rsum sgml lur dtl pra -C det sbhist hist $outname";
    if ($Lang =~ /^(mandarin)$/){ 
	if ($ManEncoding eq "utf-8"){
            $command .= " -c NOASCII DH -e utf-8";}
	else {
