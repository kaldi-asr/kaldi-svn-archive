...
if [ -f exp/tri5a/.done ]; then
  ...
  if [ ! -f $decode/.done ]; then
    ...
    local/lattice_to_ctm.sh --min_lmwt 8 --max_lmwt 14 --cmd "$train_cmd" $data $graph $decode
    touch $decode/.done
  fi
  ...
  paste -d ' ' <(cut -f 1-4 -d ' ' ${opt_ctm}.sorted ) \
      <(cut -f 5-  -d ' ' ${opt_ctm}.sorted | uconv -f utf8 -t utf8 -x Any-Lower ) > ${opt_ctm} 
  cp ${opt_ctm} ${dataset_id}.sat.ctm 
fi
...
if [ -f exp/tri6_nnet_mpe/.done ]; then
  for epoch in $opt_mpe_epoch; do
    ...
    if [ ! -f ${decode}_large/.done ]; then
      ...
      local/lattice_to_ctm.sh --min_lmwt 8 --max_lmwt 14 --cmd "$train_cmd" $data $graph ${decode}_large
    ...
    paste -d ' ' <(cut -f 1-4 -d ' ' ${opt_ctm}.sorted ) \
        <(cut -f 5-  -d ' ' ${opt_ctm}.sorted | uconv -f utf8 -t utf8 -x Any-Lower ) > ${opt_ctm} 
    cp ${opt_ctm} ${dataset_id}.dnn_mpe.ctm 
  done     
fi        
