...
if [ $# -ne 4 ]; then
  echo "Usage: $0 [options] <data-dir> <model-dir> <temp-dir> <output-dir>"
  echo " Options:"
  ...
  echo 
  echo "e.g.:"
  echo "$0 data/dev10h exp/tri4b_seg exp/tri4b_resegment_dev10h data/dev10h.seg"
  exit 1
fi
...
model_dir=$2    # Segmentation model directory created using local/resegment/train_segmentation.sh
                ...
                # To create the phone LM graph, use steps/make_phone_graph.sh.
...
local/resegment/segmentation.py --verbose 2 $segmentation_opts \
  --channel1-file A --channel2-file B --original-segments-file $datadir/segments \
  $temp_dir/pred $temp_dir/phone_map.txt 2>$temp_dir/log/resegment.log | \
  sort -k 1,1 > $output_dir/segments || exit 1
