...
def read_segments_file(segments_file):
  try:
    ...
    sys.stderr.write("Unable to open segments file %s\n" % segments_file);
    sys.exit(1);

  segments = dict();
  for line in segments_handle.readlines():
    splits = line.strip().split();
    if (len(splits) != 4):
      sys.stderr.write("Unable to parse line '%s' in %s\n" % (line, segments_file));
      sys.exit(1)
    segments[splits[0]] = (splits[1],float(splits[2]),float(splits[3]))
...
class JointResegmenter:
  def __init__(self, P, A, f, options, phone_map, stats = None, reference = None, original_segments = None):
  ...
  def print_segments(self, out_file_handle = sys.stdout):
    ...
    if self.original_segments != None:
      max_end_time_hundredths_second = int (100.0 * max([ self.original_segments[s][2] for s in self.original_segments]) )
...
def main():
  ...
  parser.add_argument('--original-segments-file', type=str, default=None, \
      help="Original segments file. This means this script will resegment the original segment file.");
  ...
  if options.original_segments_file != None:
    original_segments = read_segments_file(options.original_segments_file);
  ...
  for f in pred_files:
    if pred_files[f]:
      continue
    if not options.isolated_resegmentation:
      if re.match(".*"+channel1_file, f) is None:
        if re.match(".*"+channel2_file, f) is None:
          sys.stderr.write("%s does not match pattern .*%s or .*%s\n" \
              % (f,channel1_file, channel2_file))
          sys.exit(1)
        else:
          f1 = f
          f2 = f
          f1 = re.sub("(.*)"+channel2_file, r"\1"+channel1_file, f1)
      else:
        f1 = f
        f2 = f
        f2 = re.sub("(.*)"+channel1_file, r"\1"+channel2_file, f2)
    ...
    if options.isolated_resegmentation or f2 not in pred_files or f1 not in pred_files:
      ...
      if temp_dir != None:
        ...
        reference = None
      r = JointResegmenter(A, B, f, options, phone_map, stats, reference, original_segments)
      r.resegment()
