...
GetOptions("fragmentMarkers=s" => \$fragMarkers, 
           "hesitationToken=s" => \$Hesitation,
           "oovToken=s" => \$OOV_symbol,
           "writeReco2File" => \$write_reco2file_and_channel,
           );

sub substitue_text {
  my $orig_text=$_[0];
  my @words=split( " ", $orig_text);
  my @new_words;
  foreach $word (@words) {
    if ( $word =~ /\p{Han}/ ) {
      push @new_words, $word;
      ...
      die "Unknown/inconsistent format: $orig_text\n";
    } else {
      push @new_words, "($word)";
    }
  }
...
while ($line=<SCP>) {
    ...
    $waveform{$recordingID} =~ s:\.(sph|wav|flac)\s*$::i; # remove file extension
#    $waveform{$recordingID} =~ tr/[a-z]/[A-Z]/;
...
binmode STM, ":utf8";
$numSegments = 0;
print STM ";; CATEGORY \"0\" \"\" \"\"\n";
print STM ";; LABEL \"O\" \"Overall\" \"All Segments in the test set\"\n";
foreach $segmentID (sort keys %fileID) {
    if (exists $waveform{$fileID{$segmentID}}) {
	printf STM ("%s %s %s %.6f %.6f",
	...
	printf STM (" %s\n", substitue_text($transcription{$segmentID}));
	++$numSegments;
    } else {
...
################################################################################
if ( $write_reco2file_and_channel ) {
  ...
  print STDERR "$0: Not writing the file $reco2ctmFile (because requested to do so)\n";
}
