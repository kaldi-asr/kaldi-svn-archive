...
steps/align_fmllr.sh \
  --boost-silence 0.5 --nj $train_nj --cmd "$train_cmd" \
...
steps/align_sgmm2.sh \
  --nj $train_nj  --cmd "$train_cmd" --transform-dir exp/tri5b_ali \
  --use-graphs true --use-gselect true \
  data/train data/lang exp/sgmm5b exp/sgmm5b_ali

steps/make_denlats_sgmm2.sh \
  --nj $train_nj --sub-split $train_nj --num-threads 4 --parallel-opts "-pe smp 4"\
