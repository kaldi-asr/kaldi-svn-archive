...
EOF
  ...
  parallel_opts="--gpu 1 --config conf/no_k20.conf"
  decode_cmd="queue.pl --max-jobs-run 64 -l arch=*64,mem_free=2G,ram_free=2G"
  num_threads=1
  minibatch_size=512
...
if [ $stage -le 7 ]; then
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
    utils/create_split_dir.pl \
     /export/b0{3,4,5,6}/$USER/kaldi-data/egs/malach_en-$(date +'%m_%d_%H_%M')/s5/$dir/egs/storage $dir/egs/storage
  fi
  ...
  steps/nnet2/train_multisplice_accel2.sh --stage $train_stage \
    ...
    --mix-up 12000 \
    data/train_hires data/lang exp/nnet2_online/tri4/ $dir  || exit 1;
fi
...
if [ $stage -le 8 ]; then
  ...
  for decode_set in dev; do
      ...
      steps/online/nnet2/extract_ivectors_online.sh --cmd "$decode_cmd " --nj $num_jobs \
...
if [ $stage -le 9 ]; then
  ...
  for decode_set in dev ; do
      ...
      steps/nnet2/decode.sh --nj $num_jobs --cmd "$decode_cmd" --config conf/decode.config \
        ...
        exp/nnet2_online/nnet_ms_a/graph data/${decode_set}_hires $dir/decode_${decode_set} || exit 1;
  done
fi
...
if [ $stage -le 11 ]; then
  ...
  for decode_set in dev ; do
    ...
    steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $num_jobs \
      exp/nnet2_online/nnet_ms_a/graph data/${decode_set}_hires ${dir}_online/decode_${decode_set} || exit 1;
  done
fi
...
if [ $stage -le 12 ]; then
  ...
  for decode_set in dev ; do
      num_jobs=`cat data/${decode_set}_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
      steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $num_jobs \
        --per-utt true exp/nnet2_online/nnet_ms_a/graph data/${decode_set}_hires ${dir}_online/decode_${decode_set}_utt || exit 1;
  done
fi
...
if [ $stage -le 13 ]; then
  ...
  for decode_set in dev ; do
      num_jobs=`cat data/${decode_set}_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
      steps/online/nnet2/decode.sh --config conf/decode.config --cmd "$decode_cmd" --nj $num_jobs \
        --per-utt true --online false exp/nnet2_online/nnet_ms_a/graph data/${decode_set}_hires \
