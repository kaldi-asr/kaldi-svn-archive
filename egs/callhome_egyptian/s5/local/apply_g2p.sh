...
if ! $with_probs ; then
  ...
  cut -f 1,3 $output_lex.with_probs | awk '{if (NF > 1) print $0; }' > $output_lex
...
if [ $nlex -ne $nwlist ] ; then
  ...
  diff <(cut -f 1 $output_lex | sort -u ) \
       <(cut -f 1 $output/wordlist.orig.txt | sort -u ) | tee $output/g2p.failed | head || true
  echo "WARNING: See the rest of the diff in $output/g2p.failed"
fi
exit 0
