...
if [ $stage -le 1 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 200000 \
    $wsj_dir/data/train_si84 256 $wsj_dir/exp/tri3b exp/nnet2_online/wsj_diag_ubm
fi

if [ $stage -le 2 ]; then
  # Get the extractor by a larger data set
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    --ivector-dim 50 \
    $wsj_dir/data/train_si284 exp/nnet2_online/wsj_diag_ubm exp/nnet2_online/wsj_extractor || exit 1;
fi

if [ $stage -le 3 ]; then
  steps/online/nnet2/extract_ivectors_online2.sh --cmd "$train_cmd" --nj 30 \
    $wsj_dir/data/train_si284 exp/nnet2_online/wsj_extractor exp/nnet2_online/ivectors_train_si284 || exit 1;
  steps/online/nnet2/extract_ivectors_online2.sh --cmd "$train_cmd" --nj 4 \
    data/train exp/nnet2_online/wsj_extractor exp/nnet2_online/ivectors_train_rm || exit 1;
fi

if [ $stage -le 4 ]; then
  steps/nnet2/train_pnorm_multi.sh --stage $train_stage \
    --splice-width 7 \
    --feat-type raw \
    --online-ivector-dir1 exp/nnet2_online/ivectors_train_rm \
    --online-ivector-dir2 exp/nnet2_online/ivectors_train_si284 \
    ...
    $wsj_dir/data/train_si284 $wsj_dir/data/lang $wsj_dir/exp/tri4b_ali_si284 \
    $dir || exit 1;
fi
...
if [ $stage -le 5 ]; then
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 4 \
    data/test exp/nnet2_online/wsj_extractor exp/nnet2_online/ivectors_test || exit 1;
fi
