...
public:
    ...
    explicit mat(MatrixIndexT r, MatrixIndexT c, const T& value = T()) { Resize(r, c value); }
    
    mat(const mat& m) { CopyFromMat(m); } // copy constructor
    ~mat() { //  cout<<"destructor called"<<endl;
        free(data_);
    }
    ...
    void CopyFromMat(const mat<T> & M);
    void Transpose(const mat<T> & M);
    void MatMat(const mat<T> & M1, const mat<T> & M2);
private:
    iter  data_;
...
template<typename T>
void mat<T>::CopyFromMat(const mat<T> & M) {
...
template<typename T>
void mat<T>::Transpose(const mat<T> & M){
...
template<typename T>
void mat<T>::MatMat(const mat<T> & M1, const mat<T> & M2){
...
template <typename T>
mat<T>& vec<T>::operator = (const mat& rhs) {
    if (mat<T>::NumRows() != rhs.NumRows() || mat<T>::NumCols() != rhs.NumCols()) {
        Resize(rhs.NumRows(), rhs.NumCols());
    }
    mat<T>::CopyFromMat(rhs);
    return *this;
}

template<typename T>
short mat<T>::SSE4_dot_product(unsigned char *x, signed char *y, MatrixIndexT length)
{
    int i;
    __m128i a, b, c, lo, hi;
    __m128i *e, *f;
