...
void UnitTestGroupSoftmaxComponent() {
  std::vector<int32> group_sizes;
  int32 num_sizes = 1 + rand() % 5;
  for (int32 i = 0; i < num_sizes; i++) 
    group_sizes.push_back(100);

  {
    GroupSoftmaxComponent component;
    component.Init(group_sizes);
    UnitTestGenericComponentInternal(component);
  }
  {
    GroupSoftmaxComponent component;
    const char *str = "group-sizes=100:100:100";
    component.InitFromString(str);
    UnitTestGenericComponentInternal(component);
  }
}
