...
Component* Component::NewComponentOfType(const std::string &component_type) {
  ...
  } else if (component_type == "GroupSoftmaxComponent") {
    ans = new GroupSoftmaxComponent();
...
void GroupSoftmaxComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<GroupSoftmaxComponent>", "<GroupSizes>");
  ReadIntegerVector(is, binary, &group_sizes_);
  ExpectToken(is, binary, "<Dim>");
  ReadBasicType(is, binary, &dim_);
  ExpectToken(is,binary, "<NumActiveGroups>");
  ReadBasicType(is, binary, &num_active_groups_);
  std::string tok; // TODO: remove back-compatibility code.
  ReadToken(is, binary, &tok);
  if (tok == "<ValueSum>") {
    value_sum_.Read(is, binary);
    ...
    ExpectToken(is, binary, "<Count>");
    ReadBasicType(is, binary, &count_);
    ExpectToken(is, binary, "</GroupSoftmaxComponent>");
  } else if (tok == "<Counts>") { // Back-compat code for SoftmaxComponent.
    value_sum_.Read(is, binary); // Set both value_sum_ and deriv_sum_ to the same value,
    // and count_ to its sum.
    count_ = value_sum_.Sum();
...
void GroupSoftmaxComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<GroupSoftmaxComponent>");
  WriteToken(os, binary, "<GroupSizes>");
  WriteIntegerVector(os, binary, group_sizes_);
  WriteToken(os, binary, "<Dim>");
  WriteBasicType(os, binary, dim_);
  WriteToken(os, binary, "<NumActiveGroups>");
  WriteBasicType(os, binary, num_active_groups_);
  WriteToken(os, binary, "<ValueSum>");
  value_sum_.Write(os, binary);
  WriteToken(os, binary, "<DerivSum>");
  deriv_sum_.Write(os, binary);
...
std::string GroupSoftmaxComponent::Info() const {
  int32 active_output_dim = 0;
  for (int32 i = 0; i < num_active_groups_; i++)
    active_output_dim += group_sizes_[i];
  std::stringstream stream;
  stream << Type() << ", input-dim=" << dim_ 
         << ", output-dim=" << dim_
         << ", active output-dim=" << active_output_dim
         << ", group-sizes=";
  int32 size = group_sizes_.size();
  for (int32 i = 0; i < size-1; i++)
    stream << group_sizes_[i] << ":";
  stream << group_sizes_[size-1];
  return stream.str();
}
void GroupSoftmaxComponent::InitFromString(std::string args) {
  std::string orig_args(args);
  std::vector<int32> group_sizes;
  bool ok = ParseFromString("group-sizes", &args, &group_sizes);
  if (!ok || !args.empty() || group_sizes.size() == 0) 
    KALDI_ERR << "Invalid initializer for layer of type "
              << Type() << ": \"" << orig_args << "\"";
  Init(group_sizes);
}

void GroupSoftmaxComponent::Init(std::vector<int32> group_sizes) {
  group_sizes_ = group_sizes;
  int32 groups_sum = 0;
  num_active_groups_ = group_sizes_.size();
  for (int32 i = 0; i < num_active_groups_; i++)
     groups_sum += group_sizes[i];
  dim_ = groups_sum;
}
void GroupSoftmaxComponent::SetActiveGroups(int32 num_active_groups) {
  KALDI_ASSERT(num_active_groups <= group_sizes_.size());
  num_active_groups_ = num_active_groups;
}

int32 GroupSoftmaxComponent::OutputDim() const {
  int32 output_dim = 0;
  for (int32 i = 0; i < num_active_groups_; i++) 
    output_dim += group_sizes_[i];
  return output_dim;
}

void GroupSoftmaxComponent::Propagate(const CuMatrixBase<BaseFloat> &in,
                                      int32, 
                                      CuMatrix<BaseFloat> *out) const {
  //out->Resize(in.NumRows(), in.NumCols(), kUndefined);
  int32 active_size = 0;
  for (int32 i = 0; i < num_active_groups_; i++) 
    active_size += group_sizes_[i];
  out->Resize(in.NumRows(), active_size);
  int32 num_groups = group_sizes_.size();
  KALDI_ASSERT(in.NumCols() == dim_);
  int32 start_group_ind = 0;
  // Compute softmax of each group of outputs serapately.
  for (int32 group = 0; group < num_active_groups_; group++) {
    CuSubMatrix<BaseFloat> sub_group(in, 0, in.NumRows(), 
                                     start_group_ind, group_sizes_[group]);
    out->Range(0, in.NumRows(), start_group_ind, group_sizes_[group]).ApplySoftMaxPerRow(sub_group); 
    start_group_ind += group_sizes_[group];
  }
  out->ApplyFloor(1.0e-20);
}
void GroupSoftmaxComponent::Backprop(const CuMatrixBase<BaseFloat> &,
                                     const CuMatrixBase<BaseFloat> &out_value,
                                     const CuMatrixBase<BaseFloat> &out_deriv,
                                     int32 num_chunks,
                                     Component *to_update, // only thing updated is counts_.
  ...
  KALDI_ASSERT(SameDim(out_value, out_deriv) && SameDim(out_value, *in_deriv));
  int32 start_group_ind = 0,
    num_groups = group_sizes_.size();
  KALDI_ASSERT(out_value.NumCols() == dim_);
  for (int32 group = 0; group < num_active_groups_; group++) {
    const CuSubMatrix<BaseFloat> P(out_value, 0, out_value.NumRows(), 
                                   start_group_ind, group_sizes_[group]),
                                 E(out_deriv, 0, out_deriv.NumRows(),
                                   start_group_ind, group_sizes_[group]);
    CuSubMatrix<BaseFloat> D(*in_deriv, 0, in_deriv->NumRows(), 
                             start_group_ind, group_sizes_[group]);
    start_group_ind += group_sizes_[group];
    D.CopyFromMat(P);
    D.MulElements(E); // D = P .* E (in matlab notation)
    CuVector<BaseFloat> pe_vec(D.NumRows()); // For each row i, the dot product (p_t . e_t).
    pe_vec.AddDiagMatMat(1.0, P, kNoTrans, E, kTrans, 0.0);  
    D.AddDiagVecMat(-1.0, pe_vec, P, kNoTrans, 1.0); // does D -= diag(pe_vec) * P. 
  }
  // The GroupSoftmaxComponent does not have any real trainable parameters, but
  // during the backprop we store some statistics on the average counts;
  // these may be used in mixing-up.
  if (to_update != NULL) {
    NonlinearComponent *to_update_nonlinear =
