...
void ProcessLattice(const string& cmd,
    const fst::VectorFst<Arc>& fst, ostream* out) {
  vector<string> cmds;
...
int main(int argc, char *argv[]) {
  try {
    ...
    std::string wspecifier = po.NumArgs() == 3 ? po.GetArg(3) : "-";

    ofstream ofs;
    ostream* out = &cout;
    if (wspecifier != "-") {
      ofs.open(wspecifier.c_str());
      out = &ofs;
    }
