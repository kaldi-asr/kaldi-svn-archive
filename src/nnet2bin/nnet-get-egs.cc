  ...
  try {
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!pdf_post_reader.HasKey(key)) {
        ...
        // Modify ProcessFile in order to accept pdf_post and feats with different length
        int32 min_len = std::min(feats.NumRows(), static_cast<int>(pdf_post.size()));
        Posterior pdf_post2(pdf_post);
        Matrix<BaseFloat> feats2(feats);
        if (pdf_post.size() != feats.NumRows()) {
          KALDI_WARN << "Posterior has wrong size " << pdf_post.size()
                     << " versus " << feats.NumRows();
          num_err++;
          feats2.Resize(min_len, feats.NumCols(), kCopyData);
          pdf_post2.resize(min_len);
          //continue;
        }
        ProcessFile(feats2, pdf_post2, key,
