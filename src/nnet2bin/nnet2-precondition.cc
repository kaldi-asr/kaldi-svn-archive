...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "quantities obtained by the program nnet2-get-preconditioner.  This is useful in batch\n"
        ...
        "\n"
        "Usage:  nnet2-precondition [options] <preconditioner-in> <model-in> <model-out> [<preconditioner-out>]\n"
        "\n"
        "e.g.:\n"
    ...
    po.Register("normalize", &normalize, "If true, normalize each AffineComponent "
                "(or descendant) to have a \"standard\" parameter variance.");
    config.Register(&po);
