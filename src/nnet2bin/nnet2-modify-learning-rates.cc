...
int main(int argc, char *argv[]) {
  try {
    ...
    if (!raw) {
      ...
      ReadKaldiObject(prev_nnet_rxfilename, &prev_nnet);
    }
    ...
    if (!raw) {
      ...
      ReadKaldiObject(cur_nnet_rxfilename, &cur_nnet);
    }
    ...
    int32 num_updatable = (raw ? prev_nnet.NumUpdatableComponents() : am_prev_nnet.GetNnet().NumUpdatableComponents());
    Vector<BaseFloat> relative_diff(num_updatable);
    {
      Nnet diff_nnet((raw ? prev_nnet : am_prev_nnet.GetNnet()));
      diff_nnet.AddNnet(-1.0, (raw ? cur_nnet : am_cur_nnet.GetNnet()));
      ...
      if (!raw) {
        ...
        prev_nnet.ComponentDotProducts(prev_nnet, &baseline_prod);
      }
    ...
    if (!raw) {
      ...
      prev_nnet.GetLearningRates(&prev_nnet_learning_rates);
      cur_nnet.GetLearningRates(&cur_nnet_learning_rates);
    }
    ...
    if (retroactive) {
      ...
      if (!raw) {
        ...
        cur_nnet.AddNnet(-1.0, prev_nnet);
        cur_nnet.ScaleComponents(scale_factors);
        cur_nnet.AddNnet(1.0, prev_nnet);
      }
    ...
    if (!raw) {
      ...
      cur_nnet.SetLearningRates(nnet_learning_rates);
      SetMaxChange(0.0, &(cur_nnet));
      WriteKaldiObject(cur_nnet, modified_cur_nnet_rxfilename, binary_write);
    }
    return ret;
