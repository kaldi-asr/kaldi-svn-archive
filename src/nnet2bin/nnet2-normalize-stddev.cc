...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage: nnet2-normalize-stddev [options] <model-in> <model-out>\n"
        " e.g.: nnet2-normalize-stddev final.mdl final.mdl\n";
    ...
    for (int32 c = 0; c < nnet.NumComponents() - 2; c++) {
      ...
      component = &(nnet.GetComponent(c + 1));
      ...
      component = &(nnet.GetComponent(c + 2));
    ...
    for (int32 c = 0; c < identified_components.size(); c++) {
      Component *component = 
          &(nnet.GetComponent(identified_components[c]));
