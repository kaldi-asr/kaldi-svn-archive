...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("truncate-groups", &truncate_groups,"If set, will truncate the first \"truncate_groups\" groups"
                "in GroupSoftmax component and remove the last groups");
    po.Read(argc, argv);
    ...
    if (truncate_groups >= 0) {
      int32 nc = (raw ? nnet.NumComponents() : am_nnet.GetNnet().NumComponents());
      std::string component_type = (raw ? nnet.GetComponent(nc-1).Type() :
                                    am_nnet.GetNnet().GetComponent(nc-1).Type());
      Component *component = (raw ? &(nnet.GetComponent(nc-1)) : &(am_nnet.GetNnet().GetComponent(nc-1)));
      if (truncate_groups > 1 )
        KALDI_ASSERT(component_type == "GroupSoftmaxComponent");
      if (component_type == "GroupSoftmaxComponent") {
        GroupSoftmaxComponent *gc = dynamic_cast<GroupSoftmaxComponent*>(component);
        gc->SetActiveGroups(truncate_groups);
      }
    } 
