...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
      ...
      "lattice-wspecifier\n"
      "e.g.: lattice-expand-ngram --n=4 ark:lat ark:expanded_lat\n";
    ...
    po.Register("test", &test, "If true, verify that the lattices have unique left context after expanding.");
    ...
    for (; !lat_reader.Done(); lat_reader.Next()) {
      ...
      if (test) {
        ...
        if (HasUniquePhoneContext(lat, trans_model, n))
          ...
          KALDI_LOG << "Before expanding, lattice does not have unique left context.";
      }
