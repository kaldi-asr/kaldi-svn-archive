...
class DecodableAmNnet: public DecodableInterface {
 public:
  DecodableAmNnet(const TransitionModel &trans_model,
    ...
    // temporary cpu matrix to pass to NnnetComputation
    CuMatrix<BaseFloat> log_probs_tmp(log_probs_);
    NnetComputation(am_nnet.GetNnet(), feats, spk_info, pad_input, &log_probs_tmp);
    // copy back the temporary matrix to the log_probs_
    log_probs_tmp.CopyToMat(&log_probs_);
    
    log_probs_.ApplyFloor(1.0e-20); // Avoid log of zero which leads to NaN.
    log_probs_.ApplyLog();
    Vector<BaseFloat> priors(am_nnet.Priors().Vec());
