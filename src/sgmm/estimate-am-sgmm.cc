...
void MleAmSgmmAccs::Check(const AmSgmm &model, bool show_properties) const {
	...
	if (Z_.size() == 0) {
		...
		debug_str << "gamma_s: yes, " << string(gamma_s_nz ? "nonzero. "
				: "zero. ");
	}
...
BaseFloat MleAmSgmmAccs::AccumulateFromPosteriors(const AmSgmm &model,
		const SgmmPerFrameDerivedVars& frame_vars,
		const Matrix<BaseFloat> &posteriors, const VectorBase<BaseFloat> &v_s, // may be empty
	...
	Vector<BaseFloat> xt_jmi(feature_dim_), mu_jmi(feature_dim_), zt_jmi(
			spk_space_dim_);
...
void MleAmSgmmAccs::CommitStatsForSpk(const AmSgmm& model, const VectorBase<
		BaseFloat> &v_s) {
...
void MleAmSgmmUpdater::Update(const MleAmSgmmAccs &accs, AmSgmm *model,
		SgmmUpdateFlagsType flags) {
	...
	if (flags & (kSgmmPhoneVectors | kSgmmWeightProjections)
		...
		model->ComputeH(&H);
		ComputeSmoothingTerms(accs, *model, H, &H_sm, (flags
				& kSgmmPhoneVectors) ? &y_sm : NULL);
	}
	...
	KALDI_LOG
			<< "*Auxiliary function improvement, combining all parameters, is "
			<< (tot_impr);

	KALDI_LOG << "***Total data likelihood is " << (accs.total_like_
			/ accs.total_frames_) << " over " << (accs.total_frames_)
			<< " frames.";
...
void MleAmSgmmUpdater::Update(const MleAmSgmmAccs &accs, AmSgmm *model,
		std::vector<std::vector<int32> > &contexts, std::vector<std::vector<
				std::vector<int32> > > &questions, std::vector<std::vector<
				int32> > *subset, std::vector<Vector<double> > *vectors,
		int max_number_subsets, SgmmUpdateFlagsType flags) {
	KALDI_ASSERT((flags & (kSgmmPhoneVectors | kSgmmPhoneProjections |
							kSgmmWeightProjections | kSgmmCovarianceMatrix | kSgmmSubstateWeights |
							kSgmmSpeakerProjections)) != 0);
	...
	if (flags & kSgmmPhoneVectors) {
		if (update_options_.check_v) {
			...
			tot_impr += UpdatePhoneVectors(accs, model, H, H_sm, y_sm,
					contexts, questions, subset, vectors, max_number_subsets);
		}
	}
	if (flags & kSgmmPhoneProjections) {
		if (update_options_.compress_m_dim == 0)
		...
		for (int32 j = 0; j < accs.num_states_; ++j) {
			for (int32 m = 0; m < model.NumSubstates(j); ++m) {
				if (accs.gamma_[j](m, i) > 0.0) {
					Q_[i].AddVec2(
							static_cast<BaseFloat> (accs.gamma_[j](m, i)),
							model.v_[j].Row(m));
				}
			}
		}
	...
	for (int32 i = 0; i < accs.num_gaussians_; ++i) {
		...
		for (int32 j = 0; j < accs.num_states_; ++j) {
			for (int32 m = 0; m < model.NumSubstates(j); ++m) {
				...
				mu_jmi.AddMatVec(1.0, model.M_[i], kNoTrans,
						model.v_[j].Row(m), 0.0);
				S_means_[i].AddVec2(
						static_cast<BaseFloat> (accs.gamma_[j](m, i)), mu_jmi);
			}
		}
...
void MleAmSgmmUpdater::ComputeSmoothingTerms(const MleAmSgmmAccs &accs,
		const AmSgmm &model, const vector<SpMatrix<double> > &H, SpMatrix<
				double> *H_sm, Vector<double> *y_sm) const {
	KALDI_ASSERT(H_sm != NULL);
	H_sm->Resize(accs.phn_space_dim_);
	if (y_sm != NULL)
		y_sm->Resize(accs.phn_space_dim_);
	...
	for (int32 j = 0; j < accs.num_states_; ++j) {
		for (int32 m = 0, end = model.NumSubstates(j); m < end; ++m) {
			...
			if (y_sm != NULL)
				(*y_sm).AddVec(1.0, accs.y_[j].Row(m));
		}
	}
...
double MleAmSgmmUpdater::UpdatePhoneVectors(const MleAmSgmmAccs &accs,
		AmSgmm *model, const vector<SpMatrix<double> > &H, const SpMatrix<
				double> &H_sm, const Vector<double> &y_sm) {
	...
	for (int32 j = 0; j < accs.num_states_; ++j) {
		...
		for (int32 m = 0; m < model->NumSubstates(j); ++m) {
			...
			w_jm.AddMatVec(1.0, Matrix<double> (model->w_), kNoTrans, Vector<
					double> (model->v_[j].Row(m)), 0.0);
			w_jm.ApplySoftMax();
			...
			double objf_impr_with_prior = SolveQuadraticProblem(H_jm_dash,
					g_jm_dash, &vhat_jm,
					static_cast<double> (update_options_.max_cond),
					static_cast<double> (update_options_.epsilon), "v", true);

			SpMatrix<BaseFloat> H_jm_flt(H_jm);

			double objf_impr_noprior = (VecVec(vhat_jm, g_jm) - 0.5 * VecSpVec(
					vhat_jm, H_jm, vhat_jm)) - (VecVec(model->v_[j].Row(m),
					g_jm) - 0.5 * VecSpVec(model->v_[j].Row(m), H_jm_flt,
					model->v_[j].Row(m)));
			model->v_[j].Row(m).CopyFromVec(vhat_jm);
			if (j < 3 && m < 2) {
				KALDI_LOG << "Objf impr for j = " << (j) << " m = " << (m)
						<< " is " << (objf_impr_with_prior / (gamma_jm
						+ 1.0e-20)) << " (with ad-hoc prior) "
		...
		if (j < 10) {
			KALDI_LOG << "Objf impr for state j = " << (j) << "  is "
					...
					<< " (with ad-hoc prior) " << (state_likeimpr
					/ (state_count + 1.0e-20)) << " (no prior) over "
					<< (state_count) << " frames";
		}
	}

	KALDI_LOG << "**Overall objf impr for v is " << (totimpr
			/ (count + 1.0e-20)) << "(with ad-hoc prior) " << (likeimpr
			/ (count + 1.0e-20)) << " (no prior) over " << (count) << " frames";
	// Choosing to return actual likelihood impr here.
	return likeimpr / (count + 1.0e-20);
}

std::vector<std::pair<Vector<double> , SpMatrix<double> > > MleAmSgmmUpdater::precomputeQuantities(
		const MleAmSgmmAccs &accs, AmSgmm *model, const std::vector<SpMatrix<
				double> > &H, const SpMatrix<double> &H_sm,
		const Vector<double> &y_sm, std::vector<std::vector<int32> > contexts) {
	std::vector < std::pair<Vector<double> , SpMatrix<double> > > quantities;
	int32 num_substates = 1; // in this case we do not use sub state, so num_substate = 1

	for (int32 j = 0; j < accs.num_states_; ++j) {
		Vector<double> w_jm(accs.num_gaussians_);
		SpMatrix<double> H_jm(accs.phn_space_dim_);
		Vector<double> g_jm(accs.phn_space_dim_);
		...
		std::pair<Vector<double> , SpMatrix<double> > tmp(g_jm, H_jm);
...
std::vector<int32> MleAmSgmmUpdater::getYes(
		std::vector<std::vector<int32> > contexts, std::vector<int32> set,
		std::vector<int32> questions, int32 position) {
	std::vector<int32> answer;
	for (int32 s = 0; s < set.size(); s++) {
		if (find(questions.begin(), questions.end(), contexts[set[s]][position])
				!= questions.end()) {
			answer.push_back(set[s]);
		}
	}
	return answer;
}
...
double MleAmSgmmUpdater::UpdatePhoneVectors(const MleAmSgmmAccs &accs,
		AmSgmm *model, const vector<SpMatrix<double> > &H, const SpMatrix<
		double> &H_sm, const Vector<double> &y_sm, std::vector<
		std::vector<int32> > &contexts, std::vector<std::vector<
		std::vector<int32> > > &questions, std::vector<std::vector<
		int32> > *subSets,
		std::vector<Vector<double> > *currentSubSetVector,
		int max_number_subsets) {
	KALDI_LOG << "Updating phone vectors";

	std::vector < std::vector<int32> > context2SubSet(contexts.size()); // list of list of sub sets which contain it
	std::vector < std::vector<int32> > allCandidateSubSets; // list candidate subsets which is a list of contexts
	std::vector <Vector<double> > cacheZ; // caching all z_j = y_j - H_j * v_j
	std::vector < std::vector<int32> > subSetNew;
	int32 K = max_number_subsets;
	int32 s_index = 0; // sub set index in sub set
	int32 topN = 10;// each time we choose only topN best subsets
	int32 thresh = 1000; // only subset that has more then thresh statistics could be added to the candidate list
	double totimpr = 0.0; // sum over all states
	double cutoff = 1.0e-06;

	int32 subSetsInit = subSets->size();

	std::vector < std::pair<Vector<double> , SpMatrix<double> > > quantities
	= precomputeQuantities(accs, model, H, H_sm, y_sm, contexts);

	// Insert first all subsets in the actual SubSets to ensure in the next steps that the new candidate subset is not included in the list of all candidate sub sets
	allCandidateSubSets.insert(allCandidateSubSets.end(), (*subSets).begin(), (*subSets).end());

	// LOAD subset and state vector and generate their candidate subsets
	if (subSets->size() > 0) {
		KALDI_LOG << "Update context to subset";
		for (int32 s = 0; s < subSets->size(); s++) {
			// update context to sub set
			for (int32 c = 0; c < (*subSets)[s].size(); c++) {
				context2SubSet[(*subSets)[s][c]].push_back(s);
			}
		}
		s_index = subSets->size();
		if (subSets->size() < K) {
			KALDI_LOG
			<< "Generate all candidate subsets for all existing subsets";
			for (int32 s = 0; s < subSets->size(); s++) {
				// Generate all candidate subsets
				for (int32 qc = 0; qc < questions.size(); ++qc) {
					for (int32 qu = 0; qu < questions[qc].size(); ++qu) {
						std::vector<int32> yesContext = getYes(contexts,
								(*subSets)[s], questions[qc][qu], qc);
						if (yesContext.size() > 0 && yesContext.size()
								< (*subSets)[s].size()) {
							int32 count = 0;
							for (int32 j = 0; j < yesContext.size(); ++j) {
								Vector<double> tmp = quantities[j].first;
								if (not tmp.IsZero(cutoff)) {
									count++;
								}
							}
							if (count > thresh) {
								if (find(allCandidateSubSets.begin(), allCandidateSubSets.end(), yesContext) == allCandidateSubSets.end()) {
									allCandidateSubSets.push_back(yesContext);
								}
							}
						}
					}
		...
		KALDI_LOG << "There are now " << allCandidateSubSets.size()
						<< " candidate subsets";
		for (int32 j = 0; j < contexts.size(); ++j) {
			Vector<double> z_j(accs.phn_space_dim_); // z_j = y_j - H_j * v_j
			Vector<double> tmp = quantities[j].first;
			Vector<double> vhat_j(accs.phn_space_dim_);
			vhat_j.CopyFromVec(model->v_[j].Row(0));
			tmp.AddSpVec(-1.0, quantities[j].second, vhat_j, 1.0);
			z_j.AddVec(1.0, tmp);
			cacheZ.push_back(z_j);
		}
	}

	// MAIN: Increase #subset, update state vectors for each subset and also context
	if (s_index < K) {
		while (s_index < K) {
			KALDI_LOG << "Create new sub sets";
			double best_like_improve = 0;
			Vector<double> g_x(accs.phn_space_dim_);
			SpMatrix<double> H_x(accs.phn_space_dim_);
			KALDI_LOG << " size: " << subSets->size() << " "
					<< currentSubSetVector->size();
			// 1. Step: Generate the first sub set and its candidate sub sets
			if (subSets->size() == 0) {
				// for the first sub set
				KALDI_LOG << "The first sub set has " << contexts.size()
								<< " contexts.";
				std::vector<int32> subset_k;
				int32 count = 0;
				for (int32 j = 0; j < contexts.size(); ++j) {
					Vector<double> tmp = quantities[j].first;
					if (not tmp.IsZero(cutoff)) {
					...
					Vector<double> tmp2(accs.phn_space_dim_);
					tmp2.CopyFromVec(model->v_[j].Row(0));
					tmp.AddSpVec(-1.0, quantities[j].second, tmp2, 1.0);
					g_x.AddVec(1.0, tmp);
					H_x.AddSp(1.0, quantities[j].second);
					subset_k.push_back(j);
					context2SubSet[j].push_back(0); // the first subset
				}
				// compute x
				H_x.InvertDouble();
				KALDI_LOG << "There are only " << count
						<< " contexts with statistic.";
				Vector<double> x(accs.phn_space_dim_);
				x.AddSpVec(1.0, H_x, g_x, 0.0);
				subSets->push_back(subset_k);
				subSetNew.push_back(subset_k);
				currentSubSetVector->push_back(x);

				KALDI_LOG << "Generate all candidate sub sets of new "
						<< subSetNew.size() << " subsets.";
				for (int32 s = 0; s < subSetNew.size(); s++) {
					// Generate all candidate subsets
					for (int32 qc = 0; qc < questions.size(); ++qc) {
						for (int32 qu = 0; qu < questions[qc].size(); ++qu) {
							std::vector<int32> yesContext = getYes(contexts,
									subSetNew[s], questions[qc][qu], qc);
							if (yesContext.size() > 0 && yesContext.size()
									< subSetNew[s].size()) {
								int32 count = 0;
								for (int32 j = 0; j < yesContext.size(); ++j) {
				...
				KALDI_LOG << "There are now " << allCandidateSubSets.size()
								<< " candidate sub sets.";
				KALDI_LOG << "Update all v_j";
				for (int32 j = 0; j < contexts.size(); ++j) {
					Vector<double> vhat_j(model->v_[j].Row(0));
					Vector<double> z_j(accs.phn_space_dim_); // z_j = y_j - H_j * v_j
					std::vector<int32> existSubSets = context2SubSet[j];
					//KALDI_LOG << "There are " << existSubSets.size() << " sub set for context " << j;
					for (int32 i = 0; i < existSubSets.size(); i++)
						vhat_j.AddVec(1.0, (*currentSubSetVector)[existSubSets[i]]);
					vhat_j.Scale(1.0 / existSubSets.size());
					model->v_[j].Row(0).CopyFromVec(vhat_j);
					//KALDI_LOG << "Update v done for " << j << " context";
					Vector<double> tmp = quantities[j].first;
					tmp.AddSpVec(-1.0, quantities[j].second, vhat_j, 1.0);
					z_j.AddVec(1.0, tmp);
					cacheZ.push_back(z_j);
					//KALDI_LOG << "Compute hashZ done";
				}
				s_index = subSets->size();
			} else {
				KALDI_LOG << "Go through all " << allCandidateSubSets.size()
								<< " candidates to find the best one!";
				std::vector < std::pair<Vector<double> , SpMatrix<double> >
				> quantities_tmp;
				std::vector < std::pair<BaseFloat, int32> > best_pairs;
				// Go through all candidates but ignore the first subSets.size() to find the NEW best one
				for (int32 j = subSetsInit; j < allCandidateSubSets.size(); ++j) {
					Vector<double> g_x_tmp(accs.phn_space_dim_);
					SpMatrix<double> H_x_tmp(accs.phn_space_dim_);
					int32 size = allCandidateSubSets[j].size();
					int32 count = 0;
					if (size > 0) {
						double sum_gamma = 0;
						for (int32 l = 0; l < allCandidateSubSets[j].size(); ++l) {
							int32 cont = allCandidateSubSets[j][l];
							if (not cacheZ[cont].IsZero(cutoff)) {
								count++;
							}
							g_x_tmp.AddVec(1.0, cacheZ[cont]);
							H_x_tmp.AddSp(1.0, quantities[cont].second);
							double gamma_j = accs.gamma_[cont].Row(0).Sum(); // only 1 state for 1 context
							sum_gamma += gamma_j;
						}
						KALDI_LOG << "The candidate " << j << " has " << count << " statistics.";
						std::pair<Vector<double> , SpMatrix<double> > temp(
								g_x_tmp, H_x_tmp);
						quantities_tmp.push_back(temp);
						double like_improve = 0.5 * VecSpVec(g_x_tmp, H_x_tmp,
								g_x_tmp)/ sum_gamma;
						best_pairs.push_back(std::make_pair(like_improve, j));
					}
				}
				std::nth_element(best_pairs.begin(), best_pairs.end() - topN,
						best_pairs.end());
				best_pairs.erase(best_pairs.begin(), best_pairs.end() - topN);
				for (int32 b = 0; b < best_pairs.size(); b++) {
					Vector<double> x(accs.phn_space_dim_);
					std::vector<int32> subset_k =
							allCandidateSubSets[best_pairs[b].second];
					quantities_tmp[b].second.InvertDouble();
					x.AddSpVec(1.0, quantities_tmp[b].second,
							quantities_tmp[b].first, 0.0);
					subSets->push_back(subset_k);
					subSetNew.push_back(subset_k);
					for (int32 c = 0; c < subset_k.size(); ++c) {
						context2SubSet[subset_k[c]].push_back(subSets->size()
								- 1);
					}
					currentSubSetVector->push_back(x);
					KALDI_LOG << b << "th subset give " << best_pairs[b].first
							<< " improvement with candidate " << best_pairs[b].second << " which has "
							<< allCandidateSubSets[best_pairs[b].second].size()
							<< " contexts.";
					allCandidateSubSets[best_pairs[b].second].clear();
				}
				quantities_tmp.clear();

				KALDI_LOG << "Now there are " << subSets->size() << " subsets!";
				s_index = subSets->size();

				// update all v_k, v_j and the cache for z_j

				KALDI_LOG << "Update all v_k";
				for (int32 k = 0; k < subSets->size(); k++) {
					Vector<double> g_x(accs.phn_space_dim_);
					SpMatrix<double> H_x(accs.phn_space_dim_);
					int32 count = 0;
					for (int32 l = 0; l < (*subSets)[k].size(); ++l) {
						int32 cont = (*subSets)[k][l];
						if (not quantities[cont].first.IsZero(cutoff)) {
							count++;
							g_x.AddVec(1.0, cacheZ[cont]);
							H_x.AddSp(1.0, quantities[cont].second);
						}
					}
					KALDI_LOG << "There are " << (*subSets)[k].size()
									<< " context with " << count
									<< " statistics in sub set " << k;
					H_x.InvertDouble();
					Vector<double> delta(accs.phn_space_dim_);
					delta.AddSpVec(1.0, H_x, g_x, 0.0);
					(*currentSubSetVector)[k].AddVec(1.0, delta);
				}
				KALDI_LOG << "Update all v_j";
				for (int32 j = 0; j < contexts.size(); ++j) {
					Vector<double> vhat_j(accs.phn_space_dim_);
					...
					for (int32 i = 0; i < existSubSets.size(); i++)
						vhat_j.AddVec(1.0,
								(*currentSubSetVector)[existSubSets[i]]);
					vhat_j.Scale(1.0 / existSubSets.size());
					model->v_[j].Row(0).CopyFromVec(vhat_j);
					// Update v done for " << j << " context";
					Vector<double> tmp = quantities[j].first;
					tmp.AddSpVec(-1.0, quantities[j].second, vhat_j, 1.0);
					z_j.AddVec(1.0, tmp);
					cacheZ.push_back(z_j);
				...
				// generate all candidate sub set of a new sub set
				if (subSets->size() < K) {
					KALDI_LOG << "Generate all candidate sub sets of new "
							<< subSetNew.size() << " subsets.";
					for (int32 s = 0; s < subSetNew.size(); s++) {
						...
						for (int32 qc = 0; qc < questions.size(); ++qc) {
							for (int32 qu = 0; qu < questions[qc].size(); ++qu) {
								std::vector<int32> yesContext = getYes(
										contexts, subSetNew[s],
										questions[qc][qu], qc);
								if (yesContext.size() > 0 && yesContext.size()
										< subSetNew[s].size()) {
									int32 count = 0;
									for (int32 j = 0; j < yesContext.size(); ++j) {
										int32 cont = yesContext[j];
										if (not cacheZ[cont].IsZero(cutoff)) {
											count++;
										}
									}
			...
			totimpr += best_like_improve;
		}
	} else {
		// The specified number of subset is reached, so no increase #subset anymore
		// Then go through all the subSets and update v_k an v_j
		KALDI_LOG << "Update all v_k";
		//double impr = 0;
		for (int32 k = 0; k < subSets->size(); k++) {
			Vector<double> g_x(accs.phn_space_dim_);
			...
			KALDI_LOG << "There are " << (*subSets)[k].size()
							<< " context with " << count << " statistics in sub set "
							<< k;
			H_x.InvertDouble();
			Vector<double> delta(accs.phn_space_dim_);
			delta.AddSpVec(1.0, H_x, g_x, 0.0);
...
double MleAmSgmmUpdater::UpdatePhoneVectorsChecked(const MleAmSgmmAccs &accs,
		AmSgmm *model, const vector<SpMatrix<double> > &H) {
	...
	for (int32 j = 0; j < accs.num_states_; ++j) {
		...
		for (int32 m = 0; m < model->NumSubstates(j); ++m) {
			...
			for (backtrack_iter = 0; backtrack_iter < max_backtrack; backtrack_iter++) {
				// w_jm = softmax([w_{k1}^T ... w_{kD}^T] * v_{jkm})  eq.(7)
				Vector<double> w_jm(accs.num_gaussians_);
				w_jm.AddMatVec(1.0, Matrix<double> (model->w_), kNoTrans, v_jm,
						0.0);
				w_jm.Add(-w_jm.LogSumExp()); // it is now log w_jm

				exact_objf = VecVec(w_jm, accs.gamma_[j].Row(m)) + VecVec(v_jm,
						accs.y_[j].Row(m)) - 0.5 * VecSpVec(v_jm, X_jm, v_jm);
				...
				if (backtrack_iter == 0.0)
					...
					if (exact_objf >= exact_objf_start) {
						...
						KALDI_LOG
								<< "Backtracking computation of v_jm for j = "
								<< j << " and m = " << m
								<< " because objf changed by " << (exact_objf
								- exact_objf_start) << " [vs. predicted:] "
								<< auxf_impr;
				...
				if (backtrack_iter == 0) { // computing updated value.
					...
					for (int32 i = 0; i < accs.num_gaussians_; ++i) {
						double gamma_jmi = accs.gamma_[j](m, i);
						double quadratic_term = std::max(gamma_jmi, gamma_jm
								* w_jm(i));
						double scalar = gamma_jmi - gamma_jm * w_jm(i)
								+ quadratic_term * VecVec(model->w_.Row(i),
										model->v_[j].Row(m));
						g_jm.AddVec(scalar, model->w_.Row(i));
						if (quadratic_term > 1.0e-10) {
							H_jm.AddVec2(
									static_cast<BaseFloat> (quadratic_term),
									model->w_.Row(i));
						}
					}
					auxf_impr = SolveQuadraticProblem(H_jm, g_jm, &v_jm,
							static_cast<double> (update_options_.max_cond),
							static_cast<double> (update_options_.epsilon), "v",
							true);
				}
			}
			...
			if (j < 3 && m < 2) {
				KALDI_LOG << "Objf impr for j = " << (j) << " m = " << (m)
						<< " is " << objf_impr
						<< " vs. quadratic auxf impr (before backtrack) "
						<< auxf_impr;
			}
		}
	}

	KALDI_LOG << "**Overall objf impr for v is " << (tot_objf_impr / (tot_count
			+ 1.0e-20)) << " (auxf impr before backtracking:) "
			<< (tot_auxf_impr / (tot_count + 1.0e-20)) << " over " << tot_count
			<< " frames";
	// Choosing to return actual likelihood impr here.
...
void MleAmSgmmUpdater::RenormalizeV(const MleAmSgmmAccs &accs, AmSgmm *model,
	...
	if (!Sigma.IsPosDef()) {
		KALDI_LOG
				<< "Not renormalizing v because scatter is not positive definite"
				<< " -- maybe first iter?";
		return;
	}
	...
	KALDI_LOG
			<< "Note on the next diagnostic: the first number is generally not that meaningful "
	...
	for (int32 j = 0; j < accs.num_states_; ++j) {
		for (int32 m = 0; m < model->NumSubstates(j); ++m) {
			...
			tmp.AddMatVec(1.0, Trans, kNoTrans, Vector<double> (
					model->v_[j].Row(m)), 0.0);
	...
	for (int32 i = 0; i < accs.num_gaussians_; ++i) {
		Vector<double> tmp(accs.phn_space_dim_);
		tmp.AddMatVec(1.0, TransInv, kTrans, Vector<double> (model->w_.Row(i)),
				0.0);
...
double MleAmSgmmUpdater::UpdateM(const MleAmSgmmAccs &accs, AmSgmm *model) {
	...
	for (int32 i = 0; i < accs.num_gaussians_; ++i) {
		...
		if (gamma_i < accs.feature_dim_) {
			KALDI_WARN << "For component " << i
					<< ": not updating M due to very " << "small count (="
					<< gamma_i << ").";
			continue;
		}

		Matrix<double> Mi(model->M_[i]);
		double impr = SolveQuadraticMatrixProblem(Q_[i], accs.Y_[i], SpMatrix<
				double> (model->SigmaInv_[i]), &Mi,
				static_cast<double> (update_options_.max_cond),
				static_cast<double> (update_options_.epsilon), "M", true);
		model->M_[i].CopyFromMat(Mi);

		if (i < 10) {
			KALDI_VLOG(2) << "Objf impr for projection M for i = " << i
						<< ", is " << (impr / (gamma_i + 1.0e-20)) << " over "
						<< gamma_i << " frames";
		}
		totcount += gamma_i;
		tot_like_impr += impr;
	...
	KALDI_LOG
			<< "Overall objective function improvement for model projections "
...
double MleAmSgmmUpdater::UpdateMCompress(const MleAmSgmmAccs &accs,
	...
	SgmmCompressM compress(model->M_, accs.Y_, model->SigmaInv_, Q_, gamma,
			update_options_.compress_m_dim);
...
double MleAmSgmmUpdater::UpdateNCompress(const MleAmSgmmAccs &accs,
	...
	SgmmCompressM compress(model->N_, accs.Z_, model->SigmaInv_, accs.R_,
			gamma, update_options_.compress_n_dim);
...
double MleAmSgmmUpdater::UpdateWParallel(const MleAmSgmmAccs &accs,
	...
	for (int iter = 0; iter < update_options_.weight_projections_iters; iter++) {
		...
		for (int32 j = 0; j < accs.num_states_; j++) {
			for (int32 m = 0; m < model->NumSubstates(j); m++) {
				...
				// w_jm = softmax([w_{k1}^T ... w_{kD}^T] * v_{jkm})  eq.(7)
				w_jm.AddMatVec(1.0, w, kNoTrans, Vector<double> (
						model->v_[j].Row(m)), 0.0);
				...
				for (int32 i = 0; i < accs.num_gaussians_; i++) {
					...
					double linear_term = accs.gamma_[j](m, i) - gamma_jm
							* w_jm(i);
		...
		for (step_size = 1.0; step_size >= min_step; step_size /= 2) {
			...
			for (int32 i = 0; i < accs.num_gaussians_; i++) {
				...
				SolveQuadraticProblem(F_i[i], g_i.Row(i), &delta_w,
						static_cast<double> (update_options_.max_cond),
						static_cast<double> (update_options_.epsilon), "w",
						true);

				delta_w.Scale(step_size);
				double predicted_impr = VecVec(delta_w, g_i.Row(i)) - 0.5
						* VecSpVec(delta_w, F_i[i], delta_w);
			...
			KALDI_VLOG(2) << "For iteration " << (iter)
						<< ", updating w gives "
						<< "predicted per-frame like impr "
						<< (k_predicted_like_impr / k_count) << ", actual "
						<< ((k_like_after - k_like_before) / k_count)
						<< ", over " << (k_count) << " frames";
			if (k_like_after < k_like_before) {
				...
				if (fabs(k_like_after - k_like_before) / k_count < 1.0e-05) {
					...
					KALDI_WARN
							<< "Halving step size for weights as likelihood did "
							<< "not increase";
				}
			} else {
				break;
	...
	KALDI_LOG << "**Overall objf impr for w is " << (tot_predicted_like_impr
			/ tot_count) << ", actual " << ((tot_like_after - tot_like_before)
			/ tot_count) << ", over " << (tot_count) << " frames";
...
double MleAmSgmmUpdater::UpdateWSequential(const MleAmSgmmAccs &accs,
	...
	KALDI_LOG
			<< "Updating weight projections [original approach, checking each"
			<< "Gaussian component].";
	...
	for (int iter = 0; iter < update_options_.weight_projections_iters; iter++) {
		...
		for (int32 i = 0; i < accs.num_gaussians_; ++i) {
			...
			for (int32 j = 0; j < accs.num_states_; j++) {
				for (int32 m = 0; m < model->NumSubstates(j); m++) {
					double this_unnormalized_weight = VecVec(w_i,
							model->v_[j].Row(m));
					double normalizer = weight_tots[j](m);
					double this_log_w = this_unnormalized_weight - normalizer,
							this_w = exp(this_log_w), substate_count =
									gamma_jm[j](m), this_count =
									accs.gamma_[j](m, i);

					double linear_term = this_count - substate_count * this_w;
					double quadratic_term = std::max(this_count, substate_count
							* this_w);
			...
			SolveQuadraticProblem(F_i, g_i, &delta_w,
					static_cast<double> (update_options_.max_cond),
					static_cast<double> (update_options_.epsilon), "w", true);
			...
			try { // In case we have a problem in LogSub.
				...
				for (step_size = 1.0; step_size >= min_step; step_size /= 2) {
					...
					for (int32 j = 0; j < accs.num_states_; j++) {
						for (int32 m = 0; m < model->NumSubstates(j); m++) {
							double old_unnorm_weight = VecVec(w_i,
									model->v_[j].Row(m)), new_unnorm_weight =
									VecVec(new_w_i, model->v_[j].Row(m)),
							...
							double old_normalizer = new_weight_tots[j](m),
									delta;
							if (new_unnorm_weight > old_unnorm_weight) {
								delta = LogAdd(0, LogSub(new_unnorm_weight
										- old_normalizer, old_unnorm_weight
										- old_normalizer));
							} else {
								delta = LogSub(0, LogSub(old_unnorm_weight
										- old_normalizer, new_unnorm_weight
										- old_normalizer));
					...
					if (observed_impr < 0.0) { // failed, so we reduce step size.
						KALDI_LOG << "Updating weights, for i = " << (i)
								<< ", predicted "
									"auxf: " << (predicted_impr
								/ (this_tot_count + 1.0e-20)) << ", observed "
								<< observed_impr / (this_tot_count + 1.0e-20)
								<< " over " << this_tot_count
								<< " frames. Reducing step size " << "to "
								<< (step_size / 2);
						if (predicted_impr / (this_tot_count + 1.0e-20)
								< 1.0e-07) {
							KALDI_WARN
						...
						if (i < 10)
							KALDI_LOG << "Updating weights, for i = " << (i)
									...
									<< ": predicted " << (predicted_impr
									/ (this_tot_count + 1.0e-20))
									<< ", observed " << (observed_impr
									/ (this_tot_count + 1.0e-20)) << " over "
									<< (this_tot_count) << " frames.";
		...
		KALDI_LOG << "For iteration " << iter
				<< ", updating w gives predicted " << "per-frame like impr "
				<< (k_delta_predicted / k_count) << ", observed "
				<< (k_delta_observed / k_count) << ", over " << (k_count)
				<< " frames";
		if (iter == 0)
			tot_count += k_count;
...
double MleAmSgmmUpdater::UpdateN(const MleAmSgmmAccs &accs, AmSgmm *model) {
	...
	for (int32 i = 0; i < accs.num_gaussians_; ++i) {
		...
		double impr = SolveQuadraticMatrixProblem(accs.R_[i], accs.Z_[i],
				SpMatrix<double> (model->SigmaInv_[i]), &Ni,
				static_cast<double> (update_options_.max_cond),
				static_cast<double> (update_options_.epsilon), "N", true);
...
void MleAmSgmmUpdater::RenormalizeN(const MleAmSgmmAccs &accs, AmSgmm *model) {
	...
	for (int32 i = 0; i < accs.num_gaussians_; i++) {
		Ntmp.AddMatMat(1.0, Matrix<double> (model->N_[i]), kNoTrans, U,
				kNoTrans, 0.0);
...
double MleAmSgmmUpdater::UpdateVars(const MleAmSgmmAccs &accs, AmSgmm *model) {
	...
	for (int32 i = 0; i < accs.num_gaussians_; ++i) {
		...
		objf_improv(i) = model->SigmaInv_[i].LogPosDefDet() - TraceSpSp(
				SpMatrix<double> (model->SigmaInv_[i]), Sigma_i_ml);
	...
	if (gamma_vec.Sum() == 0) { // If no count, use identity.
		...
		if ((tmp = covfloor.LimitCondDouble(update_options_.max_cond)) != 0) {
			KALDI_WARN
					<< "Covariance flooring matrix is poorly conditioned. Fixed "
					<< "up " << (tmp) << " eigenvalues.";
		}
	}
	...
	for (int32 i = 0; i < accs.num_gaussians_; ++i) {
		...
		if (gamma_vec(i) < update_options_.cov_diag_ratio * accs.feature_dim_) {
			...
			try {
				...
				objf_improv(i) += Sigma_i.LogPosDefDet() + TraceSpSp(SpMatrix<
						double> (model->SigmaInv_[i]), Sigma_i_ml);
				...
				if (i < 5) {
					KALDI_VLOG(2) << "objf impr from variance update ="
								<< objf_improv(i) / (gamma_vec(i) + 1.0e-20)
								<< " over " << (gamma_vec(i))
								<< " frames for i = " << (i);
				}
			} catch (...) {
...
BaseFloat MleSgmmSpeakerAccs::Accumulate(const AmSgmm &model,
		const SgmmPerFrameDerivedVars& frame_vars, int32 j, BaseFloat weight) {
	// Calculate Gaussian posteriors and collect statistics
	Matrix<BaseFloat> posteriors;
	BaseFloat log_like = model.ComponentPosteriors(frame_vars, j, &posteriors);
	posteriors.Scale(weight);
...
BaseFloat MleSgmmSpeakerAccs::AccumulateFromPosteriors(const AmSgmm &model,
		const SgmmPerFrameDerivedVars& frame_vars,
	...
	// Intermediate variables
	Vector<double> xt_jmi(feature_dim), mu_jmi(feature_dim), zt_jmi(
			spk_space_dim);
	int32 num_substates = model.NumSubstates(j);
	for (int32 ki = 0; ki < static_cast<int32> (gselect.size()); ++ki) {
		int32 i = gselect[ki];
...
void MleSgmmSpeakerAccs::Update(BaseFloat min_count, Vector<BaseFloat> *v_s,
	...
	if (v_s->Dim() != T)
		v_s->Resize(T); // will set it to zero.
	...
	double tot_objf_impr = SolveQuadraticProblem(H_s, y_s_, &v_s_dbl, 1.0e+05,
			1.0e-40, "v_s", true);
	v_s->CopyFromVec(v_s_dbl);

	KALDI_LOG << "*Objf impr for speaker vector is " << (tot_objf_impr
			/ tot_gamma) << " over " << (tot_gamma) << " frames.";
...
MleAmSgmmAccs::~MleAmSgmmAccs() {
	if (gamma_s_.Sum() != 0.0)
		KALDI_ERR
				<< "In destructor of MleAmSgmmAccs: detected that you forgot to "
